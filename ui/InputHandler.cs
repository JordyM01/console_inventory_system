...
public class InputHandler
{
    ...
    private void HandleNumericInput(ConsoleKeyInfo key, StringBuilder textBuilder, bool allowDecimal)
    {
        ...
        {
            if (decimal.TryParse(currentText, NumberStyles.Any, CultureInfo.InvariantCulture, out decimal val)) textBuilder.Clear().Append(val + 1);
            else textBuilder.Clear().Append("1");
        }
        else if (key.Key is ConsoleKey.OemMinus or ConsoleKey.Subtract)
        {
            if (decimal.TryParse(currentText, NumberStyles.Any, CultureInfo.InvariantCulture, out decimal val) && val > 0) textBuilder.Clear().Append(val - 1);
        }
