...
/// <summary>
/// Vista para eliminar productos del inventario
/// </summary>
public class DeleteProductView : IView
{
    ...
    public void Draw()
    {
        ...
        Console.Write("Producto".PadRight(30));
        Console.Write("SKU".PadRight(15));
        Console.Write("Precio".PadRight(10));
        Console.Write("Cant.");
        Console.ResetColor();
        ...
        for (int i = 0; i < products.Count; i++)
        {
            ...
            Console.Write(product.Name.PadRight(30).Substring(0, 28));
            Console.Write(product.Sku.PadRight(15));
            Console.Write($"{product.Price:C}".PadRight(10));
            Console.Write(product.Quantity.ToString().PadLeft(4));
            Console.ResetColor();
        }

        if (_isConfirmingDelete && products.Count > 0)
        {
            var productToDelete = products[_selectedIndex];
            string message = $"¿Seguro que desea eliminar '{productToDelete.Name}'?";
            string options = "[S]í / [N]o";
            DrawConfirmationDialog("Confirmar Eliminación", message, options);
        }
    }

    public IView HandleInput(ConsoleKeyInfo key)
    {
        if (_isConfirmingDelete) return HandleConfirmationInput(key);
        ...
        switch (key.Key)
        {
            ...
            case ConsoleKey.Enter or ConsoleKey.Delete:
                if (products.Count > 0) _isConfirmingDelete = true;
                break;
        }
        return this;
    }
